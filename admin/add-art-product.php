...
//featured Image
if($sprice < 1){
  echo "<script>alert('Art price cannot be less than 1.');</script>";
}else{
              ...
              <option value="<?php echo $row['ID'];?>"><?php echo $row['ArtMedium'];?></option>
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10" id="tag-container">
                      <input class="form-control"  id="tag-input" type="text" name="product_tag"/>
                    </div>
                  </div>
              </div>
            </section>
  ...
  if (event.key === 'Enter' || event.key === ',') {
    ...
    if (tags !== '') {
      ...
      if (tagList.length >= maxTags) {
        return; // Limit reached, do not create new tags
      }
...
form.addEventListener('submit', function() {
  ...
  hiddenInput.name = 'tagList';
  hiddenInput.value = JSON.stringify(tagList);
